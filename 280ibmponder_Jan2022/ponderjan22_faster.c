/*
IBM Ponder This challenge main and bonus *, Jan 22
...
Of course, clockwise and counter-clockwise rotations of the above answers will also give the same results. Also, there may be other sets 
that give the same max or min.

I wrote a C program. It completed quickly for both the main and bonus puzzles. First it stores a list of d-digit primes with all distinct 
digits. Then it loops through n-digit numbers with all distinct digits, skipping any number that is equivalent through clockwise or 
counter-clockwise rotation. For a qualifying n-digit number, it iterates through the stored list of primes and checks if the prime can be 
formed, and calculates the total circle score also. It outputs the earliest n-digit number that gave the max total circle score, and also 
the earliest n-digit number that gave the min.
