...
One solution I found for the October 2019 challenge that maximizes the desired probability is {1, 2, 12, 16, 48}. The desired conditional probability for this set of denominations is about 29.07 %.

I tweaked my C program (which uses dynamic programming approach) that I wrote for the September 2019 challenge. The program run completed very quickly.

The desired conditional probability works out to 100 divided by sum of squares of counts of amounts made by each distinct set of notes for a given set of 5 denominations. That sum of squares has to be minimum to maximize the desired conditional probability. I took the US denominations as baseline; the sum of squares is 476. If a lower sum of squares is found for some set of denominations, that sum and set of denominations become the new baseline for further iterations. As with my September program, the revised program loops through different sets containing denomination 1 and four other denominations, and considers all amounts from 2 through 99. The second denomination is constrained to a maximum of 22 since any higher second denomination would cause the sum of squares to exceed 476. During the dynamic programming based search, it checks if more than one distinct minimal set of notes could be used to make an amount. If true, the corresponding set of five denominations is rejected. The program also moves to a fresh set of denominations as soon as the sum of squares for the current set being processed is greater than or equal to the current baseline. I didn't try any other performance improvements apart from the above.

After exhausting the sets of 5 denominations, the program exited with the best solution found {1, 2, 12, 16, 48} with desired conditional probability being 29.07 %.
