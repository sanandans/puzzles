'''
IBM Ponder This challenge and bonus *, Feb 22
...
I took a recursive approach to the problem. If a number is divisible by 2, then its summands are simply 2 multiplied by the summands of half the number. If a number is not divisible by 2, we can consider the greatest power of 3 less than or equal to the number. This will be a non-conflicting summand for this number. We can reduce the number by this greatest power of 3, and get a smaller, even number. We can iterate through the smaller and smaller numbers using the same ideas. I wrote a Python program and it completed instantaneously for both the main and bonus "*" puzzles. For the mail puzzle, it produced the desired result for the very first 200-digit number of the desired kind (10^199), giving 125 summands. For the bonus "*" puzzle also, it gave the desired result for the very first 300-digit number of the desired kind (10^299), giving 200 summands. Since the summands have powers of one of the bases (2 or 3) in strictly ascending order, and the other one in strictly descending order, clearly no two summands can fully divide each other.
