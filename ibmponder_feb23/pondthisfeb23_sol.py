'''
IBM Ponder This challenge and bonus *, Feb 23
...
[(0, 10), (1, 0), (2, 2), (3, 4), (4, 1), (5, 11), (6, 8), (7, 14), (8, 19), (9, 17), (10, 15), (11, 9), (12, 3), (13, 16), (14, 18), (15, 7), (16, 12), (17, 6), (18, 13), (19, 5)]

Bonus "*" puzzle (n = 26, number of ways = 0):

[(0, 13), (1, 0), (2, 2), (3, 4), (4, 1), (5, 3), (6, 8), (7, 10), (8, 15), (9, 17), (10, 19), (11, 22), (12, 24), (13, 18), (14, 20), (15, 25), (16, 9), (17, 11), (18, 6), (19, 23), (20, 12), (21, 16), (22, 5), (23, 7), (24, 14), (25, 21)]

I wrote a python program. It ran for about 5 minutes to solve the main puzzle, and then I changed it for the bonus "*" puzzle for which the program ran in only 15 seconds. First, it generates n-queen solutions fast. For this I used bitwise operators. I used symmetry in one direction to halve the solution space, but even without the symmetry the n-queen generation was very quick. For each n-queen solution generated, it then generates the safe squares list. I have a fixed set of bitmasks for each of the left and right diagonals of an n x n board (where all the cells of each left diagonal have a distinct coordinate sum, and the same idea for right diagonals). By using the diagonal bitmasks of the n queens, I generate an ordered list of safe squares. For the main puzzle, I used backtracking to find the number of ways (upto 48) to place the 20 kings. If exactly 48 ways are found, we get the 20-queens answer for the main puzzle.
 
For the bonus "*" question (I used n = 26), I reduce the safe square list and the number of kings remaining to be placed. If a safe square is connected to zero or exactly one safe square, then a king can be put in that safe square, and the safe square can be removed from the picture. If it's connected to exactly one safe square, then that other safe square can also be removed. After this pruning, I use my backtracking function to look for one way to place the remaining kings in the remaining safe squares. If a first way to place the kings cannot be found, we get the 26-queens answer for the bonus "*" question.
...
def make_safelist(queenlist,slist,covcells,bonus):
    ...
    # But coverage matrix is (N+2)x(N+2) including margin cells to allow carefree array ops, and array rownum is top to bottom, colnum is left to right.
    # Hence put safe cells at correct places in coverage matrix. Queens grid is not relevant for kings search. Non safe cells also not relevant.
    while safecells != 0:
...
def build_neighbors(nlist, nlist2, slist, covcells, bonus):
    for safecell in slist:
        ...
        #for main puzzle, we only care about safe neighbors that are ahead of this safe cell since we will do king search backtracking in order.
        #for extra credit puzzle, we can prune the safe list by placing king on any safe cell with 0 or 1 neighbor safe cell, but we need to look all
...
#used for bonus puzzle. We can prune the safe list and remaining kings by placing king on any safe cell with 0 or 1 neighbor safe cell, but we need to look all
#around this safe cell for neighbors before we can decide if this safe cell has degree 1. We could also extend this to get the Maximum Independent Set (MIS)
#which would tell us if max kings is less than N, but just pruning the safe list and reducing the number of kings to be placed by backtracking is enough
#to solve the bonus in a performant manner.
...
def queenrecurse(leftdiag, rightdiag, col, excludes, qlist, currrow, targetcombs, bonus):
    ...
    #start n-queens search from bottom row of grid. Use excludes bitmask to exclude half of row 0 (to take advantage of at least one symmetry)
