...
One solution I found for the condition that there can be only five denominations is {1, 2, 4, 8, 62}. This set gives the desired probability of exactly 4%.

First I tried to do this with pen and paper using various sequences including geometric progressions, but then wrote a small C program that uses the dynamic programming approach. The program found the above solution almost instantaneously.

The program loops through different sets containing denomination 1 and four other denominations, and considers all amounts from 2 through 99. During the dynamic programming based search, it checks if more than one distinct minimal set of notes could be used to make an amount. The corresponding set of five denominations is rejected. When a unique minimal set of notes is found for an amount, the program increments an array of counters that keeps track of how many amounts are made minimally by each set of notes. If all amounts get processed for a given set of five denominations, the program computes the total desired permutations using the array of counters, and checks if we got 396 permutations. If not, it tries again with a new set of five denominations. The program exited on finding the first solution {1, 2, 4, 8, 62}.
*/
